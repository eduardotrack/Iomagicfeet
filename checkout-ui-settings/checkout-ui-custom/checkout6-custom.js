...
function insertMessage(message, container) {
  if (container) {
...
function findDivsAtPaymentPage() {
  ...
  let intervalId = setInterval(function() {
    ...
    if (pickupContainerAtAuth) {
      ...
      insertMessage(message, pickupContainerAtAuth);

      clearInterval(intervalId);

      changeDeliveryOption?.addEventListener('click', function() {
        let clickIntervalId = setInterval(function() {
          pickupContainerAtAuth = $('.pickup-packages')[0];
          if (pickupContainerAtAuth) {
            const message = createPickupMessage();
            insertMessage(message, pickupContainerAtAuth);
  ...
  openDeliveryOptions?.addEventListener('click', function() {
    setTimeout(function() {
      pickupContainerAtAuth = $('.pickup-packages')[0];
      const message = createPickupMessage();
      insertMessage(message, pickupContainerAtAuth);
...
$(document).ready(function () {
  let btnPlaceOrder = $('.btn-place-order')[0];

  setTimeout(() => findDivsAtPaymentPage(), 2000)

  if (btnPlaceOrder) {
    btnPlaceOrder.addEventListener('click', function() {
      setTimeout(() => findDivsAtPaymentPage(), 500)
    })
  }
})
...
$(window).on("orderFormUpdated.vtex", function(evt, orderForm) {
  ...
  if (isPickup) {
    let intervalId = setInterval(function() {
      let pickupContainerAtCart = $('.srp-pickup-info')[0];
      let pickupContainerAtAuth = $('.pickup-packages')[0];

      if (pickupContainerAtCart || pickupContainerAtAuth) {
        if (pickupContainerAtCart) {
          let message = createPickupMessage();
          insertMessage(message, pickupContainerAtCart);
        }
