...
export const SimilarProductsShelf = ({
  ...
  const totalProducts = suggestionsFiltered.length
  const slidesToShowCount = slidesToShow || (isDesktop ? 4 : 2)
  const effectiveSlidesToShow = Math.min(slidesToShowCount, totalProducts)
  const shouldInfinite = totalProducts > effectiveSlidesToShow

  const settings = {
    arrows: showArrows ?? isDesktop,
    dots: totalProducts > effectiveSlidesToShow,
    ...
    slidesToScroll: Math.max(1, effectiveSlidesToShow),
    responsive: [
      {
        breakpoint: 1024,
        settings: {
  ...
  const useFilter =
    productContext?.product?.specificationGroups[0].specifications
  ...
  if (loading) {
    return <div className={styles.customShelfSkeleton} />
  }

  const shelfContainer = isFromNotifyUnavailable
    ? document.querySelector('.magicfeet-magicapp-0-x-notifyUnavailableShelf')
  ...
  return (
    <div className={`${styles.customShelfWrapper}`}>
      {console.log('arrows-->arrows', suggestionsFiltered.length)}
    ...
    >
      ...
      >
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M6.41265 10.9174C6.8222 10.52 6.82256 9.86278 6.41344 9.46492L3.24495 6.38364C2.80447 5.95529 2.80447 5.24784 3.24495 4.81949L6.41344 1.7382C6.82256 1.34034 6.8222 0.68315 6.41265 0.285739C6.01986 -0.0954056 5.39528 -0.0954057 5.00249 0.285739L0.331102 4.81866C-0.110512 5.24718 -0.110512 5.95594 0.331103 6.38447L5.0025 10.9174C5.39528 11.2985 6.01986 11.2985 6.41265 10.9174Z"
          fill="#1C1C1C"
        />
      </svg>
    </div>
